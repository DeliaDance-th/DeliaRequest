...
function executeLogout() {
  ...
  updateUserUI(); closeModal('logoutModal');
  showToast("ออกจากระบบ", "คุณได้ออกจากระบบแล้ว", "success");
  goHome();
}
...
function renderSongs(songs) {
  ...
  songs.forEach(s => {
    ...
    let tagsHTML = ''; if (s.gender === 'M') tagsHTML += `<span class="tag tag-m">ชาย</span>`; else if (s.gender === 'F') tagsHTML += `<span class="tag tag-f">หญิง</span>`; else tagsHTML += `<span class="tag">ผสม</span>`; if (s.isBreakdance === 'Yes' || s.isBreakdance === true) tagsHTML += `<span class="tag tag-bd">Breakdance</span>`;
...
function openSongDetail(id) {
  ...
  document.getElementById('det-title').innerText = s.name; document.getElementById('det-artist').innerText = s.artist; document.getElementById('det-time').innerText = `${s.start} - ${s.end}`; document.getElementById('det-votes').innerText = `${s.votes} โหวต`;
  let tagText = s.gender === 'M' ? 'ศิลปินชาย' : (s.gender === 'F' ? 'ศิลปินหญิง' : 'วงผสม'); if (s.isBreakdance === 'Yes' || s.isBreakdance === true) tagText += ' + Breakdance'; document.getElementById('det-tag').innerText = tagText;
...
function handleVote() { if (!userUUID) { closeModal('songDetailModal'); return openModal('authModal'); } const btn = document.getElementById('btn-toggle-vote'); btn.disabled = true; btn.innerHTML = "กำลังประมวลผล..."; fetchAPI("voteSong", { eventId: activeEvent.id, songId: selectedSongId, uuid: userUUID }, res => { currentSongs = res; filterSongs(); openSongDetail(selectedSongId); }, () => { btn.disabled = false; }); }
function shareSong() { const url = window.location.origin + window.location.pathname + "?eventId=" + activeEvent.id; navigator.clipboard.writeText(url).then(() => { showToast("คัดลอกลิงก์สำเร็จ", "นำลิงก์ไปแชร์ให้เพื่อนเพื่อเข้ามาโหวตได้เลย!", "success"); }).catch(() => { showCustomConfirm("ลิงก์สำหรับแชร์", url, () => {}); }); }

async function processYoutubeLink(inputId, nameId, artistId, genderId) {
  const url = document.getElementById(inputId).value; if (!url.includes('youtu')) return; const btnId = inputId === 'song-link' ? 'btn-song-submit' : 'btn-song-edit-submit'; const btn = document.getElementById(btnId); if(btn) { btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> กำลังวิเคราะห์...`; btn.disabled = true; }
  try { const res = await fetch(`https://noembed.com/embed?dataType=json&url=${url}`); const data = await res.json(); if(data && data.title) { fetchAPI("aiProcess", { title: data.title, author: data.author_name }, dbRes => { document.getElementById(nameId).value = dbRes.song; document.getElementById(artistId).value = dbRes.artist; const gSelect = document.getElementById(genderId); if(gSelect) gSelect.value = (dbRes.gender === "M" || dbRes.gender === "F") ? dbRes.gender : "Mix"; showToast("ดึงข้อมูลสำเร็จ!", `ระบบระบุ: ${dbRes.vibe}`, "success"); if(btn) { btn.innerText = inputId === 'song-link' ? "ส่งข้อมูล" : "บันทึกการแก้ไข"; btn.disabled = false; } }, () => { if(btn) { btn.innerText = inputId === 'song-link' ? "ส่งข้อมูล" : "บันทึกการแก้ไข"; btn.disabled = false; } }); } } catch(e) { if(btn) { btn.innerText = inputId === 'song-link' ? "ส่งข้อมูล" : "บันทึกการแก้ไข"; btn.disabled = false; } }
}
...
function checkQuotaAndOpenModal() { if (!userUUID) return openModal('authModal'); if (!isAdminLoggedIn && currentSongs.filter(s => s.creator === userUUID).length >= 3) return showToast("โควตาเต็ม", "คุณเสนอเพลงครบ 3 เพลงแล้ว", "error"); ['song-link','song-name','song-artist','song-start','song-end'].forEach(id => document.getElementById(id).value = ''); document.getElementById('song-breakdance').checked = false; document.getElementById('song-gender').value = 'Mix'; openModal('addSongModal'); }
let pendingSongData = null; function preCheckAddSong() { const name = document.getElementById('song-name').value.trim(); if(!name) return showToast("ข้อมูลไม่ครบ", "กรุณาระบุชื่อเพลง", "error"); const dup = currentSongs.find(s => s.name.toLowerCase().replace(/\s/g, '') === name.toLowerCase().replace(/\s/g, '')); pendingSongData = { name: name, artist: document.getElementById('song-artist').value, link: document.getElementById('song-link').value, start: document.getElementById('song-start').value, end: document.getElementById('song-end').value, isBreakdance: document.getElementById('song-breakdance').checked, gender: document.getElementById('song-gender').value }; if (dup) showCustomConfirm("พบเพลงซ้ำ", `มีคนเสนอเพลง "${dup.name}" ไว้แล้ว คุณต้องการเสนอซ้ำหรือไม่?`, executeAddSong); else executeAddSong(); }
function executeAddSong() { const btn = document.getElementById('btn-song-submit'); btn.innerText = "กำลังส่ง..."; btn.disabled = true; fetchAPI("addSong", { eventId: activeEvent.id, songData: pendingSongData, uuid: userUUID, isAdmin: isAdminLoggedIn }, res => { currentSongs = res; filterSongs(); closeModal('addSongModal'); showToast("สำเร็จ", "เพิ่มเพลงเข้าสู่ระบบแล้ว", "success"); btn.innerText = "ส่งข้อมูล"; btn.disabled = false; }, () => { btn.innerText = "ส่งข้อมูล"; btn.disabled = false; }); }
...
function executeEditSong() { const name = document.getElementById('edit-song-name').value.trim(); if(!name) return showToast("ข้อมูลไม่ครบ", "กรุณาระบุชื่อเพลง", "error"); const updatedData = { name: name, artist: document.getElementById('edit-song-artist').value, link: document.getElementById('edit-song-link').value, start: document.getElementById('edit-song-start').value, end: document.getElementById('edit-song-end').value, isBreakdance: document.getElementById('edit-song-breakdance').checked, gender: document.getElementById('edit-song-gender').value }; const btn = document.getElementById('btn-song-edit-submit'); btn.innerText = "กำลังบันทึก..."; btn.disabled = true; fetchAPI("editSong", { eventId: activeEvent.id, songId: selectedSongId, songData: updatedData, uuid: userUUID, isAdmin: isAdminLoggedIn }, res => { currentSongs = res; filterSongs(); closeModal('editSongModal'); showToast("สำเร็จ", "อัปเดตเพลงเรียบร้อย", "success"); btn.innerText = "บันทึกการแก้ไข"; btn.disabled = false; }, () => { btn.innerText = "บันทึกการแก้ไข"; btn.disabled = false; }); }
...
// ==========================================
// 8. ระบบจัดการ Playlist
// ==========================================
let sortableLists = []; let boardSortable = null; let isPoolOpen = true;
function togglePoolSidebar() { const pool = document.getElementById('pool-sidebar'); const text = document.getElementById('toggle-sidebar-text'); isPoolOpen = !isPoolOpen; if(isPoolOpen) { pool.classList.remove('collapsed'); text.innerText = "ซ่อนกองกลาง"; } else { pool.classList.add('collapsed'); text.innerText = "แสดงกองกลาง"; } }
...
function addNewCategory() { openInputModal("ตั้งชื่อหมวดหมู่ใหม่", "K-Pop ช่วงที่ 1", (newTitle) => { if(!newTitle) return; if(newTitle.toLowerCase() === 'pool' || newTitle === 'list-pool' || newTitle === 'กองกลาง') return showToast("ชื่อนี้ไม่อนุญาต", "ชื่อนี้สงวนไว้ กรุณาใช้ชื่ออื่น", "error"); createCategoryBox(newTitle); }); }
function createCategoryBox(title) {
  ...
  col.innerHTML = `<div class="list-col-header" title="ลากสลับกล่องได้เลย"><h3>${title} <i class="fa-solid fa-pen text-muted" style="font-size:0.8rem; cursor:pointer; margin-left:8px;" onclick="renameCategory('${safeId}')" title="เปลี่ยนชื่อ"></i> <i class="fa-solid fa-trash text-danger" style="font-size:0.8rem; cursor:pointer; margin-left:4px;" onclick="deleteCategory('${safeId}')" title="ลบกล่องนี้"></i></h3><button class="btn btn-outline btn-full btn-sm" onclick="randomizeAlternate('${safeId}')"><i class="fa-solid fa-shuffle"></i> สุ่มสลับ (M/F)</button></div><div id="${safeId}" class="drop-zone" data-category="${title}"></div>`;
  document.getElementById('dynamic-board').appendChild(col); initSortables();
}
function renameCategory(zoneId) { const zone = document.getElementById(zoneId); if(!zone) return; const oldTitle = zone.getAttribute('data-category'); openInputModal("เปลี่ยนชื่อหมวดหมู่", oldTitle, (newTitle) => { if(newTitle && newTitle !== "" && newTitle.toLowerCase() !== 'pool' && newTitle !== 'กองกลาง') { zone.setAttribute('data-category', newTitle); zone.previousElementSibling.querySelector('h3').innerHTML = `${newTitle} <i class="fa-solid fa-pen text-muted" style="font-size:0.8rem; cursor:pointer; margin-left:8px;" onclick="renameCategory('${zoneId}')"></i> <i class="fa-solid fa-trash text-danger" style="font-size:0.8rem; cursor:pointer; margin-left:4px;" onclick="deleteCategory('${zoneId}')"></i>`; } }); }
function deleteCategory(zoneId) { const zone = document.getElementById(zoneId); if(!zone) return; const items = Array.from(zone.children); if(items.length > 0) { showCustomConfirm("แจ้งเตือน", "มีเพลงอยู่ในกล่อง ระบบจะย้ายกลับไปกองกลาง ยืนยันหรือไม่?", () => { const pool = document.getElementById('list-pool'); items.forEach(el => pool.appendChild(el)); zone.parentElement.remove(); }); } else zone.parentElement.remove(); }
function confirmImportFromVotes() { showCustomConfirm("รีเซ็ตจากผลโหวต", "นำเพลงทั้งหมดกลับไปที่กองกลาง ยืนยันหรือไม่?", () => { const pool = document.getElementById('list-pool'); document.querySelectorAll('.dynamic-board .drop-zone').forEach(zone => zone.innerHTML = ''); pool.innerHTML = ''; currentSongs.filter(s => s.status !== 'Played').sort((a,b) => b.votes - a.votes).forEach(s => pool.appendChild(createPlaylistItem(s))); showToast("ดึงข้อมูลสำเร็จ", "ย้ายรายชื่อทั้งหมดลงกองกลางแล้ว"); }); }
...
function aiMagicSetlist() { const pool = document.getElementById('list-pool'); const items = Array.from(pool.children); if (items.length < 3) return showToast("เพลงน้อยเกินไป", "ต้องมีอย่างน้อย 3 เพลงในกองกลาง", "error"); const btn = document.getElementById('btn-ai-setlist'); btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> กำลังคิด...`; btn.disabled = true; const songData = items.map(el => ({ id: el.dataset.id, gender: el.dataset.gender, isBreakdance: el.dataset.bd === "true" })); fetchAPI("aiMagicSetlist", { songs: songData }, sortedIds => { pool.innerHTML = ''; sortedIds.forEach(id => { const matchedEl = items.find(el => el.dataset.id === id); if (matchedEl) pool.appendChild(matchedEl); }); items.forEach(el => { if (!sortedIds.includes(el.dataset.id)) pool.appendChild(el); }); btn.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> AI`; btn.disabled = false; showToast("จัดเรียงเสร็จสิ้น!", "AI สลับเพศให้เรียบร้อยแล้ว", "success"); }, () => { btn.innerHTML = `<i class="fa-solid fa-wand-magic-sparkles"></i> AI`; btn.disabled = false; }); }
function savePlaylistData() { const lists = []; document.querySelectorAll('.drop-zone').forEach(zone => { const listName = zone.getAttribute('data-category'), items = zone.children; for (let i = 0; i < items.length; i++) { const s = currentSongs.find(song => song.id === items[i].dataset.id); if (s) lists.push({ listName: listName, id: s.id, name: s.name, artist: s.artist, gender: s.gender, isBreakdance: s.isBreakdance, time: `${s.start}-${s.end}`, link: s.link }); } }); const btnTop = document.getElementById('btn-save-playlist-top'); const btnBot = document.getElementById('btn-save-playlist-bottom'); if(btnTop) { btnTop.innerText = "กำลังบันทึก..."; btnTop.disabled = true; } if(btnBot) { btnBot.innerText = "กำลังบันทึก..."; btnBot.disabled = true; } fetchAPI("savePlaylist", { eventId: activeEvent.id, date: activeEvent.date, lists: lists }, res => { if(btnTop) { btnTop.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> เซฟลง Sheet`; btnTop.disabled = false; } if(btnBot) { btnBot.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> บันทึกลง Sheet`; btnBot.disabled = false; } showToast("บันทึกสำเร็จ", "ข้อมูล Setlist ถูกเซฟลง Sheet แล้ว"); }, () => { if(btnTop) { btnTop.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> เซฟลง Sheet`; btnTop.disabled = false; } if(btnBot) { btnBot.innerHTML = `<i class="fa-solid fa-floppy-disk"></i> บันทึกลง Sheet`; btnBot.disabled = false; } }); }
...
function handleAddEvent() {
  ...
  if (!p.date || !p.name) return showToast("ข้อมูลไม่ครบ", "กรุณากรอกวันที่และชื่องาน", "error");
  const btn = document.getElementById('btn-event-submit'); btn.innerText = "กำลังสร้าง..."; btn.disabled = true;
  fetchAPI("createEvent", p, res => { currentEvents = res; renderCalendar(); closeModal('addEventModal'); showToast("สำเร็จ", "สร้างกำหนดการเรียบร้อย", "success"); btn.innerText = "สร้างกำหนดการ"; btn.disabled = false; }, () => { btn.innerText = "สร้างกำหนดการ"; btn.disabled = false; });
}
